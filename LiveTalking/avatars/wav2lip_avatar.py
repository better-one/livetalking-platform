...
class LipReal(BaseAvatar):
    ...
    def paste_back_frame(self,pred_frame,idx:int):
        ...
        # Resize predicted face to bbox size with high-quality interpolation
        res_frame = cv2.resize(pred_frame.astype(np.uint8), (bw, bh), interpolation=cv2.INTER_LANCZOS4)

        # Color correction: match predicted face color to original
        orig_face = combine_frame[y1:y2, x1:x2]
        if orig_face.shape == res_frame.shape:
            # Match mean color per channel to reduce color shift
            for c in range(3):
                orig_mean = orig_face[:, :, c].mean()
                pred_mean = res_frame[:, :, c].mean()
                if pred_mean > 0:
                    res_frame[:, :, c] = np.clip(
                        res_frame[:, :, c].astype(np.float32) * (orig_mean / pred_mean),
                        0, 255
                    ).astype(np.uint8)

        # Feathered blending: smooth edges to avoid hard boundary
        # Only blend the outer border region (feather_size pixels)
        feather_size = max(4, min(bh, bw) // 12)
        ...
        for i in range(feather_size):
            ...
            mask[bh - 1 - i, :] = min(mask[bh - 1 - i, 0], alpha)
            mask[:, i] = np.minimum(mask[:, i], alpha)
            mask[:, bw - 1 - i] = np.minimum(mask[:, bw - 1 - i], alpha)

        mask_3ch = mask[:, :, np.newaxis]
        blended = (res_frame * mask_3ch + orig_face * (1 - mask_3ch)).astype(np.uint8)
